...
{
    //Set panel number
    public GameObject CollectedNumber3;
    ...
    public static event PuzzleComplete OnPuzzleCompleted;

    //Sound Related
    public static event EventHandler fountainWater;
    ...
    public IEnumerator CheckForComplete()
    {
        ...
        if (Complete)
        {
            ...
            OnFountainWater();
            ViveAction.magnetActive = true;
            CollectedNumber3.SetActive(true);

            hint_magnet.SetActive(true);
        }
    ...
    private void CreatePuzzleTiles()
    {
        ...
        for (int j = Height - 1; j >= 0; j--)
        {
            for (int i = 0; i < Width; i++)
            {
                ...
                TileDisplayArray[i, j].gameObject.tag = "SlidingTile";
    ...
    protected virtual void OnFountainWater()
    {
        if (fountainWater != null)
        {
            fountainWater(this, EventArgs.Empty);
        }
    }
}
